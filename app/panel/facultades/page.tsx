...
const FORM_INICIAL: Partial<Facultad> = {
  codigofacultad: "",
  nombrefacultad: "",
}
...
export default function FacultadesPage() {
  ...
  const [form, setForm] = useState<Partial<Facultad>>(FORM_INICIAL)
  ...
  const fetchData = async () => {
    ...
    .from("facultad")
    .select("*")
    .order("idfacultad", { ascending: true })

    if (error) {
      showToast("Error cargando Facultades: " + error.message, "error")
    } else {
      setFacultades(data || [])
    }
    setLoading(false)
...
, [form])
  ...
  const facultadesFiltrados = useMemo(() => facultades.filter(f => {
    const matchSearch = f.nombrefacultad?.toLowerCase().includes(search.toLowerCase()) || f.codigofacultad?.toLowerCase().includes(search.toLowerCase())
    return matchSearch
  }), [facultades, search])

  const totalPaginas = Math.ceil(facultadesFiltrados.length / registrosPorPagina)
  const indiceInicio = (paginaActual - 1) * registrosPorPagina
  const indiceFin = indiceInicio + registrosPorPagina
  const facultadesPaginados = facultadesFiltrados.slice(indiceInicio, indiceFin)
  useEffect(() => { setPaginaActual(1) }, [search])

  const limpiarFiltros = () => setSearch("")

  const handleSave = async () => {
    if (!puedeGuardar) return showToast("Complete el Nombre de Facultad *", "error");
    try {
      let mensaje = "";
      const dataToSave = {
      ...form,
                codigofacultad: form.codigofacultad?.trim().toUpperCase() || null,

      }

      if (editing) {
        const { error } = await supabase.from("facultad").update(dataToSave).eq("idfacultad", editing.idfacultad);
        if (error) throw error;
        mensaje = "Facultad actualizada correctamente";
      }
      else {
        const { error } = await supabase.from("facultad").insert(dataToSave);
        if (error) throw error;
      ...
      if (err.code === "23505")
        showToast("El Código de Facultad ya está registrado", "error")
      else
  ...
  const confirmarEliminar = async () => {
    ...
    const { error } = await supabase.from("facultad").delete().eq("idfacultad", idAEliminar)
    if (error)
      showToast("Error al eliminar: " + error.message, "error")
    else {
      showToast("Facultad eliminada correctamente", "success");
      fetchData()
    }
  ...
  return (
    <div>
      <div className="header-responsive">
        <div><h1>Registro de Facultades</h1><p>Total: {facultadesFiltrados.length} registros</p></div>
        <button className="btn-primario" onClick={() => openModal()}><Plus size={18} />Nueva Facultad</button>
      </div>
      ...
      <div className="card-sgpc" style={{ marginBottom: "2.4rem", padding: "2rem" }}>
        <div style={{ display: "flex", gap: "1.6rem" }}>
          <div style={{ position: "relative", flex: 1 }}>
            ...
            <input className="input-sgpc" placeholder="Buscar por Código o Nombre de Facultad..." value={search} onChange={e => setSearch(e.target.value)} style={{ paddingLeft: "4rem" }} />
          </div>
      ...
      <div className="card-sgpc" style={{ overflowX: "auto" }}>
        {loading? <p>Cargando...</p> : (
          <table className="tabla-sgpc">
            ...
            <tbody>{facultadesPaginados.map((f, index) => (
              <tr key={f.idfacultad}>
                <td style={{ fontWeight: 600 }}>{indiceInicio + index + 1}</td>
                <td style={{ fontWeight: 600 }}>{f.codigofacultad || '-'}</td>
                ...
                <td style={{ display: "flex", gap: "0.8rem" }}>
                  ...
                  <button className="btn-icon btn-icon-eliminar" onClick={() => handleDelete(f.idfacultad)}><Trash2 size={15} /></button>
                </td>
              </tr>
            ))}</tbody>
          </table>
      ...
      {totalPaginas > 1 && (
        <div className="paginacion-footer">
          <p className="paginacion-info">Mostrando {indiceInicio + 1} al {Math.min(indiceFin, facultadesFiltrados.length)} de {facultadesFiltrados.length} registros</p>
      ...
      {showModal && (
        <div className="modal-overlay" onClick={handleClose}>
          <div className="modal-content card-sgpc" style={{maxWidth: "60rem"}} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-header"><h2><Building2 size={20} style={{marginRight: "0.8rem"}}/>{editing? "Editar Facultad" : "Nueva Facultad"}</h2><button onClick={handleClose} className="btn-cerrar"><X size={20} /></button></div>
            <div className="modal-body">
              <div className="grid-2">
                <div className="input-wrapper">
                  <label className="input-label">Código Facultad</label>
                  <input className="input-sgpc-floating" placeholder="FAC-01" value={form.codigofacultad || ""} onChange={e => setForm({...form, codigofacultad: e.target.value.toUpperCase() })} maxLength={20} />
                </div>
                <div className="input-wrapper">
                  <label className="input-label">Nombre de Facultad *</label>
                  <input 
  className="input-sgpc-floating" 
  placeholder="FACULTAD DE MEDICINA HUMANA" 
  value={form.nombrefacultad || ""} 
  onChange={e => setForm({...form, nombrefacultad: e.target.value.toUpperCase() })} // <- agrega .toUpperCase()
  maxLength={200} 
  style={{ textTransform: 'uppercase' }} // <- y este estilo
/>
                </div>
              </div>
            </div>
      ...
      {showConfirm && (
        <div className="modal-overlay"><div className="modal-content card-sgpc" style={{ maxWidth: "40rem" }}>
          <div className="modal-header"><h2>Eliminar Facultad</h2><button onClick={() => setShowConfirm(false)} className="btn-cerrar"><X size={20} /></button></div>
          <div className="modal-body"><p style={{ textAlign: "center" }}>¿Está seguro de eliminar esta Facultad? Esta acción no se puede deshacer.</p></div>
