...
import { Plus, Edit, Trash2, X, Search, ChevronLeft, ChevronRight, Building, Eraser} from 'lucide-react'

type Filial = {
  idfilial: number
  codigofilial: string | null
  nombrefilial: string | null
}

const FORM_INICIAL: Partial<Filial> = {
  codigofilial: "",
  nombrefilial: "",
}
...
export default function FilialesPage() {
  ...
  const [form, setForm] = useState<Partial<Filial>>(FORM_INICIAL)
  ...
  const fetchData = async () => {
    ...
    const { data, error } = await supabase
     .from("filial")
     .select("*")
     .order("idfilial", { ascending: true })

    if (error) {
      showToast("Error cargando Filiales: " + error.message, "error")
    } else {
      setFiliales(data || [])
    }
    setLoading(false)
 ...
 , [form])
  ...
  const filialesFiltrados = useMemo(() => filiales.filter(f => {
    const matchSearch = f.nombrefilial?.toLowerCase().includes(search.toLowerCase()) || f.codigofilial?.toLowerCase().includes(search.toLowerCase())
    return matchSearch
  }), [filiales, search])

  const totalPaginas = Math.ceil(filialesFiltrados.length / registrosPorPagina)
  const indiceInicio = (paginaActual - 1) * registrosPorPagina
  const indiceFin = indiceInicio + registrosPorPagina
  const filialesPaginados = filialesFiltrados.slice(indiceInicio, indiceFin)
  useEffect(() => { setPaginaActual(1) }, [search])

  const limpiarFiltros = () => setSearch("")

  const handleSave = async () => {
    if (!puedeGuardar) return showToast("Complete el Nombre de Filial *", "error");
    try {
      let mensaje = "";
      const dataToSave = {
       ...form,
        codigofilial: form.codigofilial?.trim().toUpperCase() || null,
        nombrefilial: toTitleCase(form.nombrefilial?.trim() || "")
      }

      if (editing) {
        const { error } = await supabase.from("filial").update(dataToSave).eq("idfilial", editing.idfilial);
        if (error) throw error;
        mensaje = "Filial actualizada correctamente";
      }
      else {
        const { error } = await supabase.from("filial").insert(dataToSave);
        if (error) throw error;
        mensaje = "Filial registrada correctamente";
      }
      ...
      if (err.code === "23505")
        showToast("El Código de Filial ya está registrado", "error")
      else
  ...
  const confirmarEliminar = async () => {
    ...
    const { error } = await supabase.from("filial").delete().eq("idfilial", idAEliminar)
    if (error)
      showToast("Error al eliminar: " + error.message, "error")
    else {
      showToast("Filial eliminada correctamente", "success");
      fetchData()
    }
  ...
  const openModal = (item?: Filial) => {
    ...
    if (item) {
      ...
      setForm({
        ...
        nombrefilial: item.nombrefilial,
      });
    }
    setShowModal(true)
  }
  ...
  return (
    <div>
      <div className="header-responsive">
        <div><h1>Registro de Filiales</h1><p>Total: {filialesFiltrados.length} registros</p></div>
        <button className="btn-primario" onClick={() => openModal()}><Plus size={18} />Nueva Filial</button>
      </div>
      ...
      <div className="card-sgpc" style={{ marginBottom: "2.4rem", padding: "2rem" }}>
        <div style={{ display: "flex", gap: "1.6rem" }}>
          <div style={{ position: "relative", flex: 1 }}>
            ...
            <input className="input-sgpc" placeholder="Buscar por Código o Nombre de Filial..." value={search} onChange={e => setSearch(e.target.value)} style={{ paddingLeft: "4rem" }} />
          </div>
        ...
        {loading? <p>Cargando...</p> : (
          <table className="tabla-sgpc">
            <thead><tr>
              ...
              <th>CÓDIGO</th>
              <th>NOMBRE DE FILIAL</th>
              <th style={{width: "12rem"}}>ACCIONES</th>
            </tr></thead>
            <tbody>{filialesPaginados.map((f, index) => (
              <tr key={f.idfilial}>
                <td style={{ fontWeight: 600 }}>{indiceInicio + index + 1}</td>
                <td style={{ fontWeight: 600 }}>{f.codigofilial || '-'}</td>
                <td>{f.nombrefilial}</td>
                <td style={{ display: "flex", gap: "0.8rem" }}>
                  <button className="btn-icon btn-icon-editar" onClick={() => openModal(f)}><Edit size={15} /></button>
                  <button className="btn-icon btn-icon-eliminar" onClick={() => handleDelete(f.idfilial)}><Trash2 size={15} /></button>
                </td>
              </tr>
            ))}</tbody>
          </table>
      ...
      {totalPaginas > 1 && (
        <div className="paginacion-footer">
          <p className="paginacion-info">Mostrando {indiceInicio + 1} al {Math.min(indiceFin, filialesFiltrados.length)} de {filialesFiltrados.length} registros</p>
      ...
      {showModal && (
        <div className="modal-overlay" onClick={handleClose}>
          <div className="modal-content card-sgpc" style={{maxWidth: "60rem"}} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-header"><h2><Building size={20} style={{marginRight: "0.8rem"}}/>{editing? "Editar Filial" : "Nueva Filial"}</h2><button onClick={handleClose} className="btn-cerrar"><X size={20} /></button></div>
            <div className="modal-body">
              <div className="grid-2">
                <div className="input-wrapper">
                  <label className="input-label">Código Filial</label>
                  <input className="input-sgpc-floating" placeholder="FIL-01" value={form.codigofilial || ""} onChange={e => setForm({...form, codigofilial: e.target.value.toUpperCase() })} maxLength={20} />
                </div>
                <div className="input-wrapper">
                  <label className="input-label">Nombre de Filial *</label>
                  <input className="input-sgpc-floating" placeholder="FILIAL LIMA NORTE" value={form.nombrefilial || ""} onChange={e => setForm({...form, nombrefilial: e.target.value })} maxLength={200} />
                </div>
              </div>
            </div>
      ...
      {showConfirm && (
        <div className="modal-overlay"><div className="modal-content card-sgpc" style={{ maxWidth: "40rem" }}>
          <div className="modal-header"><h2>Eliminar Filial</h2><button onClick={() => setShowConfirm(false)} className="btn-cerrar"><X size={20} /></button></div>
          <div className="modal-body"><p style={{ textAlign: "center" }}>¿Está seguro de eliminar esta Filial? Esta acción no se puede deshacer.</p></div>
