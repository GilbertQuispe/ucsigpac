...
export default function RegisterPage() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      if (!persona) { toast.error('DNI no encontrado, consulte con su gestora'); setIsDniValid(false); setCheckingDni(false); return; }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!idPersona) return toast.error('No se encontró el ID de persona');
    ...
    if (authError) {
      toast.error(authError.message || 'Error al crear usuario'); // <-- FIX
      setLoading(false);
      return;
    }
      ...
      <section style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
        <div className="card-sgpc" style={{ maxWidth: '42rem', width: '100%', padding: '3.2rem' }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            ...
            <button type="button" onClick={handleCancel} className="btn-secundario" style={{flex:1, height: '4.8rem'}}>Cancelar</button>
            <button type="submit" disabled={loading || !isDniValid} className="btn-primario" style={{ flex:1, height: '4.8rem', opacity: !isDniValid ? 0.5 : 1, cursor: !isDniValid ? 'not-allowed' : 'pointer' }}>{loading ? 'Creando...' : 'Crear'}</button></div>
